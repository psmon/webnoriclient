 ...
 */
angular.
module('publicqholdemPage').
component('publicqholdemPage', {
    templateUrl: '/js/app/serverview/publicqholdem-page/publicqholdem-page.template.html',
    controller: ['$routeParams','Auth','Game','$scope','$location','$cacheFactory','$window',
        function PublicqholdemPageController($routeParams,Auth,Game,$scope,$location,$cacheFactory,$window) {
            $scope.lang=$lang;
            var me = this;
            $scope.lang=$lang;
            ...
            $scope.CloseDlg = function (obj) {
                if($scope.dlgType==2){
                    ...
                    Game.JoinPublicQHoldem($scope.gameID,obj.rhand.name,obj.gamememo2,obj.form.joinernick.$viewValue,obj.joineravatar.name ,function(isSucced,data){
                        if(isSucced){
